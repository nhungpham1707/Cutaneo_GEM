function sbmlModel = convertCobraToSBML_NhungModified(model)
...
% Nhung Modified line 68-71 in the original code to adapt to crypto model
...
%% Compartments
compNameList = {'[cell envelope]','[cytoplasm]','[extracellular]','[mitochondrion]','[nucleus]','[peroxisome]','[endoplasmic reticulum]',...
    '[vacuole]','[Golgi]','[lipid particle]','[vacuolar membrane]'}; 
compSymbolList = {'ce','c','e','m','n','x','r','v','g','l','vm'}; 
...
[tokens tmp_met_struct] = regexp(model.mets,'(?<met>^.+[a-zA-Z]*[0-9]*.*_)(?<comp>[a-z]+$)','tokens','names');
...
%     tmp_species.compartment = tmp_met_struct{i}.comp;
...
for (i=1:size(the_compartments,2))
    ...
    %tmp_symbol_index = find(strcmp(compSymbolList,tmp_id));
